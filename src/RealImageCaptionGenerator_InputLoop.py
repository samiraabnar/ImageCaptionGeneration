...
class CaptionGenerator(object):
    ...
    def init_lstm_weights(self):
      #  WordEmbedding = random_normal_matrix((self.lstm_input_dim // 2, self.output_dim))
       ...
       # self.WordEmbedding = theano.shared(value=WordEmbedding, name="WordEmbedding", borrow="True")
        ...
        I_w = random_normal_matrix((self.image_rep_dim , self.lstm_hidden_dim), scale=1.0)
        self.I_w = theano.shared(value=I_w, name="I_w", borrow="True")
        ...
        self.params = [self.U_input, self.U_forget, self.U_output, self.W_input, self.W_forget, self.W_output,
                       ...
                       self.U_2, self.W_2,
                       self.O_w,  self.ImageEmbedding, self.I_w
                       #self.WordEmbedding,
                       ]
    ...
    def define_network(self):
        ...
        imagination_hidden_state = self.rng.normal(size=(self.lstm_hidden_dim,))
        imagnation_input = T.clip(T.nnet.sigmoid(T.dot(self.I_w,imagination_hidden_state)), 1e-7, 1.0 - 1e-7)
        ...
        self.imagin = theano.function([],[imagnation_input])
...
class Experiment(object):
    def __init__(self,id,relative_test_size,number_of_items_per_combination,number_of_epochs):
        self.id = id
    ...
    def do(self):
        ...
        for e in np.arange(self.number_of_epochs):
            ...
            print("Now let's imagin:")
            [imagination_item] = talker.imagin()
            [description] = talker.describe(np.asarray(imagination_item,dtype="float32"))
            print(get_string(description,VOCAB,exp.last_index))
            [imagination_item] = talker.imagin()
            [description] = talker.describe(np.asarray(imagination_item,dtype="float32"))
            print(get_string(description,VOCAB,exp.last_index))
            ...
            for k in np.arange(len(exp.test_items)):
                ...
                print(str(k)+": "+get_string(exp.test_labels[k],VOCAB,exp.last_index)+" : "+get_string(output,VOCAB,exp.last_index))
                test_cost_ += cost
